...
export const emergencyContactSchema = z.object({
  ...
  kinship: z.enum([
    ...
    'TIO', 'SOBRINO', 'PRIMO', 'TUTOR', 'REPRESENTANTE_LEGAL', 'CUIDADOR',
    'AMIGO', 'COMPANERO_TRABAJO', 'VECINO', 'OTRO'
...
export const clinicalAntecedentSchema = z.object({
  ...
  antecedentType: z.enum([
    ...
    'INMUNOLOGICO', 'HABITO', 'PSIQUIATRICO', 'NEONATAL', 'OTRO'
...
export const patientSchema = z.object({
  ...
  culturalGroup: z.enum(['MESTIZO', 'MONTUBIO', 'INDIGENA', 'AFROECUATORIANO', 'MULATO', 'BLANCO', 'OTRO']).nullable().optional(),
  culturalGroupOther: z.string().max(100).nullable().optional(),
  maritalStatus: z.enum(['SOLTERO', 'CASADO', 'DIVORCIADO', 'VIUDO', 'UNION_LIBRE', 'SEPARADO']).nullable().optional(),
  educationLevel: z.enum(['NINGUNO', 'EDUCACION_BASICA', 'BACHILLERATO', 'TERCER_NIVEL', 'CUARTO_NIVEL']).nullable().optional(),
  ...
  infoSourceType: z.enum([
    'PACIENTE', 'FAMILIAR', 'AMIGO', 'CUIDADOR', 'TUTOR', 'REPRESENTANTE_LEGAL',
    'MEDICO_REFERENTE', 'PERSONAL_SALUD', 'EMERGENCIA_PREHOSPITALARIA',
    'HISTORIA_CLINICA', 'INFORME_MEDICO', 'EXPEDIENTE_PREVIO', 'AUTORIDAD', 'OTRO'
  ...
  if (data.culturalGroup === 'OTRO' && (!data.culturalGroupOther || data.culturalGroupOther.trim() === '')) {
    return false;
  }
  return true;
}, {
  ...
  if (data.infoSourceType === 'OTRO' && (!data.infoSourceOther || data.infoSourceOther.trim() === '')) {
    return false;
  }
  return true;
}, {
