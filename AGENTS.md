...
- Application: `CreatePrescriptionUseCase`, `UpdatePrescriptionUseCase`, `GetPrescriptionByIdUseCase`, `ListPrescriptionsByDocumentUseCase`, `EmitPrescriptionUseCase` (valida Rp obligatorio antes de emitir; la asignación del número la hace la BD), `DeletePrescriptionUseCase`.
...
- Integración: el registry (`documentRegistry.ts`) expone `supportsPrescriptions` por tipo (true en 008 y 005). Los workspaces 008 (`EvolutionWorkspacePage`) y 005 (`Form005WorkspacePage`) montan un botón "Recetas" que abre el manager vía el store. Los modales (`PrescriptionsManagerModal` + `PrescriptionReadOnlyModal`) se montan en `AppLayout`. Datos de empresa/logo: variables `VITE_COMPANY_TYPE` / `VITE_COMPANY_NAME` / `VITE_COMPANY_LOGO_URL` (igual que el header del 005).
- Backend (`supabase/migrations/`):
