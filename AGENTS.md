...
Módulos activos: `ai`, `auth`, `catalog`, `dashboard`, `document`, `evolution`, `form005`, `medical-record`, `messaging`, `notifications`, `patient`, `prescription`, `reports`, `shared`, `users`.
...
- Presentation:
  ...
  - Registry: `registry/documentRegistry.ts` — **fuente única de verdad** para label, icono, `workspacePath(patientId, documentId)` y `createLabel` por tipo. Cualquier nuevo tipo de documento se registra aquí. Exporta además `DOCUMENT_ICON` (`icon-medical-document`): **un único icono genérico para todos los documentos en listados y notificaciones** (la etiqueta/código siempre acompaña, así no hay confusión y el layout no se rompe al sumar tipos). Los iconos por tipo del registry se siguen usando en las UIs de creación/selección (menú, modal, filtro). El sidebar conserva `icon-medical-evolution`.
...
- Backend (`supabase/migrations/`):
  ...
  - `64_form005_entries_denormalize_author.sql`: **desnormaliza el autor** de cada atención (`created_by_name`, `created_by_role`) porque la RLS de `profiles` impedía que médicos/enfermería leyeran el perfil de otros usuarios (veían autor null). El trigger `guard_form005_entry` (SECURITY DEFINER) puebla el nombre/rol en el INSERT y los preserva en UPDATE; backfill de filas existentes. El repo deja de embeber `author:profiles` y el mapper usa las columnas. Mismo patrón que notificaciones/recetas.
  - NOTA: la 56 quedó como versión inicial; en bases ya migradas se aplican los incrementos en orden (59, 60, 61, 62, 63, 64). No reaplicar la 56 sobre una base existente.
...
- Notificaciones: tipo `NEW_FORM005` registrado en `notificationRegistry.ts` (icono `icon-medical-document`, ruta al workspace del 005). La metadata añade `documentStatus`. `NEW_EVOLUTION` también usa `icon-medical-document` (icono unificado de documentos).

### 6.3. Prescription (`prescription`)
Recetas médicas **genéricas** asociadas a cualquier documento clínico (008/005 y futuros) mediante el par `(sourceDocumentType, sourceDocumentId)`. Un documento puede tener **muchas recetas**. Ciclo de vida `BORRADOR` (editable por el autor) → `EMITIDA` (inmutable, numerada por año y lista para imprimir/firmar físicamente). NO aparece en el listado unificado `/documentos` ni dispara notificaciones: se gestiona únicamente desde el workspace del documento padre.

- Domain: `MedicalPrescription` (cabecera + `diagnoses: PrescriptionDiagnosis[]` + firma derivada del autor: `createdByName`/`createdBySpecialty`/`createdByProfessionalCode`), `PrescriptionDiagnosis`, `PrescriptionStatus` (`BORRADOR` | `EMITIDA`), payloads `CreatePrescriptionPayload` / `UpdatePrescriptionPayload` / `PrescriptionDiagnosisInput`, interfaz `PrescriptionRepository`.
- Application: `CreatePrescriptionUseCase`, `UpdatePrescriptionUseCase`, `GetPrescriptionByIdUseCase`, `ListPrescriptionsByDocumentUseCase`, `EmitPrescriptionUseCase` (valida Rp obligatorio antes de emitir; la asignación del número la hace la BD), `DeletePrescriptionUseCase`, `GetPrescriptionCountsByMedicalRecordUseCase` (conteo de recetas por documento de una historia, para accesos rápidos).
- Infrastructure: `SupabasePrescriptionRepository` (`create`/`update` con reemplazo total de diagnósticos, `emit`/`delete` con `.select("id")` para **verificar filas afectadas** y lanzar error si la RLS bloqueó la operación —evita "éxito silencioso"—, `getById`/`listByDocument` con embed `medical_prescription_diagnoses`; el firmante NO se obtiene por embed a `profiles` sino de columnas **denormalizadas** `created_by_name`/`created_by_specialty`/`created_by_professional_code`), `PrescriptionMapper`.
- Presentation:
  - Components: `PrescriptionsManagerModal` (lista las recetas del documento; crear/editar/emitir/vista previa; bloquea creación/edición si el documento padre está CERRADO), `form/PrescriptionFormModal` (fecha de registro, vigencia con default +3 días editable, alergias en un solo campo, CIE-10 múltiple vía `form/PrescriptionCie10Picker`, Rp e Indicaciones con `WcTextareaExpand`), `read-only/PrescriptionReadOnlyView` (documento dividido en mitades: izquierda "Prescripción Médica (Rp)." / derecha "Indicaciones", **mismo header en ambas mitades** con Logo+Company_name+Company_type+N° receta, fecha, N° HCl, paciente, edad, CIE-10, alergias, vigencia, y firma del doctor al pie) + `read-only/PrescriptionReadOnlyView.css`, `read-only/PrescriptionReadOnlyModal` (carga receta + paciente + historia; **selector de formato de impresión** + botón "Imprimir / PDF"). La vista acepta `printLayout` (`side` | `stacked`) y lo refleja como `data-print-layout` en `.rx-print-root` (WYSIWYG: la previa cambia según el modo). Impresión vía `window.print()` con `@page` con nombre: `side` → A4 **horizontal** (Rp e Indicaciones lado a lado en una hoja, recetas cortas); `stacked` → A4 **vertical** con salto de página (`page-break-after`) tras el Rp para imprimir primero Prescripción y luego Indicaciones (recetas largas). El chrome del modal y el toolbar se ocultan en impresión vía visibility/`display:none`.
  - Hooks: `usePrescriptionsByDocument`, `usePrescription`, `useCreatePrescription`, `useUpdatePrescription`, `useEmitPrescription`, `useDeletePrescription`, `usePrescriptionCountsByMedicalRecord` (devuelve un mapa `${type}:${id}` → count vía `select`; helper `prescriptionCountKey`). El manager habilita acciones por permiso: **crear** solo doctor/admin con el documento padre abierto; **editar/emitir/eliminar** solo el **autor** sobre su BORRADOR; vista previa para todos; muestra el firmante denormalizado en cada fila.
  - Store: `usePrescriptionUIStore` (`managerTarget` + `readOnlyTarget`; reseteado en `useAuth.logout`).
  - Schemas: `prescription.schema.ts` (`validatePrescription`, `buildDefaultPrescriptionValues`, `addDaysToDate`).
- Integración: el registry (`documentRegistry.ts`) expone `supportsPrescriptions` por tipo (true en 008 y 005). Los workspaces 008 (`EvolutionWorkspacePage`) y 005 (`Form005WorkspacePage`) montan un botón "Recetas" que abre el manager vía el store. Los modales (`PrescriptionsManagerModal` + `PrescriptionReadOnlyModal`) se montan en `AppLayout`. **Accesos rápidos** al manager (consulta): acción "Recetas" (`icon-prescription`) en la tabla de `/documentos` (`ClinicalDocumentsTable`, siempre habilitada) y en cada documento de la Historia Clínica (`MedicalRecordEvolutionsList`, **deshabilitada si el documento no tiene recetas** según `usePrescriptionCountsByMedicalRecord`). Datos de empresa/logo: variables `VITE_COMPANY_TYPE` / `VITE_COMPANY_NAME` / `VITE_COMPANY_LOGO_URL` (igual que el header del 005).
- Backend (`supabase/migrations/`):
  - `64_medical_prescriptions_schema.sql`: enum `prescription_status_enum`; tabla `prescription_number_counters` + función atómica `next_prescription_number(year)`; tabla `medical_prescriptions` (FK a `medical_records`, asociación genérica `source_document_type`+`source_document_id`, `issue_date`, `validity_date`, `allergies`, `rp_text`, `indications_text`, número por año, `created_by`→profiles, `emitted_at`); tabla hija `medical_prescription_diagnoses` (CIE-10 múltiple, código+descripción denormalizados). Triggers: `prepare_prescription_insert` (setea `created_by` y `validity_date = issue_date + 3` por defecto), `guard_prescription_mutation` (bloquea UPDATE/DELETE cuando EMITIDA y, en la transición BORRADOR→EMITIDA, asigna `prescription_year`/`prescription_seq`/`prescription_number = YYYY-NNNNNN` + `emitted_at`), `guard_prescription_diagnosis` (diagnósticos mutables solo mientras el padre es BORRADOR). RLS inicial + realtime.
  - `65_prescriptions_rls_and_signer.sql`: **corrige RLS y firma**. (1) Agrega columnas denormalizadas del firmante (`created_by_name`/`created_by_specialty`/`created_by_professional_code`), las backfillea y extiende `prepare_prescription_insert` (SECURITY DEFINER) para poblarlas desde `profiles` en cada INSERT — así los **no-admin** pueden ver quién firmó sin chocar con la RLS de `profiles`. (2) Modelo de permisos definitivo: **SELECT** abierto a `authenticated`; **INSERT** solo `doctor`/`admin` y como uno mismo (`created_by = auth.uid()`); **UPDATE/DELETE** solo el autor (`created_by = auth.uid()`); las EMITIDA siguen inmutables por `guard_prescription_mutation`, por lo que DELETE solo aplica a BORRADOR del autor.
